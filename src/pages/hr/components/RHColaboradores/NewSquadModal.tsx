...
export function NewSquadModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150" onClick={onClose}>
      <div className="w-full max-w-xl bg-[var(--color-surface)] border border-white/10 rounded-2xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 slide-in-from-bottom-2 duration-200" onClick={e => e.stopPropagation()}>
        <div className="px-8 pt-8 pb-5 border-b border-white/5 flex items-center justify-between">
          <div>
            <div className="text-base font-semibold text-white">Novo Squad</div>
            <div className="text-xs text-slate-400">Crie um time para qualquer área da empresa</div>
          </div>
        ...
        <form onSubmit={onSubmit} className="px-8 py-6 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs text-slate-400 mb-1.5 block">Nome do Squad</label>
          ...
          <div className="space-y-2">
            <label className="text-xs text-slate-400 mb-1.5 block">Cor de Identificação</label>
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-slate-400 mb-1.5 block">Gestor do Squad</label>
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-slate-400 mb-1.5 block">Emblema / Logo <span className="text-slate-600 font-normal">(opcional)</span></label>
            <label className="flex items-center gap-4 p-4 border border-dashed border-white/10 rounded-xl cursor-pointer hover:border-blue-500/40 transition-all group">
              ...
              <div>
                <p className="text-sm text-slate-300 group-hover:text-white transition-colors">{newSquadLogo ? "Trocar imagem" : "Carregar imagem"}</p>
                <p className="text-xs text-slate-500">JPG, PNG ou GIF até 2MB</p>
              </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-slate-400 mb-1.5 block">Descrição & Objetivo</label>
