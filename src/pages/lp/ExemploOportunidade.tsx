...
export function ExemploOportunidadeSection() {
  return (
    <Section id="exemplo" className="bg-white/[0.015]">
      ...
      <div className="grid lg:grid-cols-2 gap-8 items-center">
        ...
        <FadeIn delay={0.15}>
          <GlassCard className="p-6 sm:p-7 space-y-4">
            <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-emerald-400 mb-1">
              <Sparkles className="w-3.5 h-3.5" /> Análise do Axis
            </div>

            {[
              { icon: TrendingUp, label: "Intenção de compra", value: "Alta", tone: "text-emerald-400" },
              { icon: Calendar, label: "Prazo", value: "Este mês", tone: "text-blue-300" },
              { icon: Target, label: "Contexto", value: "Possível troca de sistema", tone: "text-slate-200" },
              { icon: CheckCircle2, label: "Próxima ação", value: "Aprofundar necessidade e agendar diagnóstico", tone: "text-slate-200" },
            ].map((row) => (
              <div key={row.label} className="flex items-start gap-3 py-2 border-b border-white/[0.06] last:border-0">
                <row.icon className="w-4 h-4 text-slate-500 mt-0.5 shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-0.5">{row.label}</p>
            ...
            <div className="pt-1">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-400/20 text-[11px] font-bold text-blue-300">
      ...
      <div className="mt-12 text-center max-w-lg mx-auto">
        ...
        <p className="text-[11px] text-slate-600 mt-4 uppercase tracking-wider font-bold">Exemplo ilustrativo</p>
      </div>
    </Section>
  );
}
