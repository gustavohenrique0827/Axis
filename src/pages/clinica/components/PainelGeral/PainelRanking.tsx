      ...
      <Card className="lg:col-span-3 p-8 bg-[var(--color-surface-elevated)] border border-[var(--color-border-default)]">
        ...
        {doctorRanking.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 gap-4 opacity-40">
            ...
            <p className="text-[11px] font-black text-[var(--color-text-muted)] uppercase tracking-widest text-center">Nenhum médico cadastrado ainda.</p>
          </div>
        ) : (
            ...
            {doctorRanking.map((dr, i) => (
              <div key={i} className="grid grid-cols-4 items-center p-4 bg-[var(--color-surface-sunken)] rounded-2xl border border-[var(--color-border-default)] hover:border-emerald-500/40 transition-all group">
                <div className="col-span-2 flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-black text-[var(--color-text-primary)]">{dr.name}</p>
                    <p className="text-[10px] text-[var(--color-text-muted)] font-bold uppercase italic">Especialista</p>
                  </div>
                </div>
                <div className="text-center">
                  <p className="text-[9px] text-[var(--color-text-muted)] font-black uppercase mb-1">Pacientes</p>
                  <p className="text-sm font-black text-[var(--color-text-primary)] font-mono">{dr.patients}</p>
                </div>
