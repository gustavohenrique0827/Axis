...
export function DeploysChart() {
  return (
    <Card className="p-8 bg-[var(--color-surface-elevated)]/80 border-white/5">
      ...
      {deployData.length === 0 ? (
        <div className="h-[260px] flex items-center justify-center text-slate-600 text-xs font-bold uppercase tracking-widest text-center px-6">
          Nenhum registro de deploy disponível ainda.
        </div>
      ) : (
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={deployData}>
            ...
            />
            <Bar dataKey="deploys" fill="#2563EB" radius={[6, 6, 0, 0]} name="Deploys" />
          </BarChart>
        </ResponsiveContainer>
      )}
    </Card>
