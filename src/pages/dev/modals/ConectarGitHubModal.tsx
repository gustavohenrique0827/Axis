      ...
      <div className="w-full max-w-md bg-[var(--color-surface-elevated)] border border-white/10 rounded-2xl shadow-2xl overflow-hidden ring-1 ring-black/5 animate-in fade-in zoom-in-95 slide-in-from-bottom-2 duration-200">
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-white">Conectar GitHub</h2>
              <p className="text-xs text-slate-500">Personal Access Token</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-white/[0.03] border border-white/10 rounded-xl p-4 space-y-2.5">
            <p className="text-xs text-slate-400">Como criar o token</p>
            <ol className="space-y-1.5">
              {[
                ...
                <li key={i} className="flex items-start gap-2 text-xs text-slate-400">
                  <span className="w-4 h-4 rounded-full bg-white/10 text-slate-300 flex items-center justify-center text-[10px] shrink-0 mt-0.5">
                    {i + 1}
                  </span>
                  {step}
                </li>
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-white transition-colors mt-1"
            >
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-slate-400 block">Token</label>
            <div className="relative">
              <input
                ...
                className="w-full bg-[var(--color-surface-elevated)] border border-white/8 rounded-xl px-4 py-3 pr-11 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-white/30 font-mono"
              />
              <button
                type="button"
        ...
        <div className="flex items-center justify-end gap-3 p-6 pt-0">
          <Button variant="outline" onClick={onClose} className="h-9 px-5 text-xs border-white/8">
            Cancelar
          </Button>
          <Button
            onClick={handleConnect}
            disabled={loading || !pat.trim()}
            className="h-9 px-5 text-xs bg-white text-black hover:bg-slate-200 gap-2"
          >
