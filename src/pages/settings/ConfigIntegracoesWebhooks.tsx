            ...
            {globalWebhooks.map((w, idx) => (
              <div key={w.id} className="bg-[var(--color-surface)] p-3 rounded-xl border border-white/5 space-y-1.5 text-xs">
                <div className="flex justify-between items-center column-gap">
                  <span className="text-xs text-slate-400">{w.event}</span>
                  <div className="flex gap-2 items-center">
                     <button onClick={() => toggleGlobalWebhook(w.id)} className="text-xs px-2 py-0.5 flex items-center gap-1.5 text-slate-400">
                ...
                <div className="text-xs font-mono text-slate-400 truncate">{w.endpoint}</div>
                <div className="pt-2 flex justify-end gap-2">
                  <button onClick={() => testWebhookTrigger(idx)} className="text-xs text-slate-400 hover:underline">Disparar Teste</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2 text-xs font-mono">
            {webhookLogs.map((log, idx) => (
              <div key={idx} className="p-2.5 bg-[var(--color-surface)] border border-white/5 rounded-lg flex items-start gap-3">
                <span className="px-1.5 py-0.5 bg-emerald-500/10 text-emerald-400 rounded">{log.status}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-xs text-slate-500">{log.time} • Endpoint {log.endpoint}</div>
