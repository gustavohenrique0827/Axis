...
export function ClientesList({
  ...
  return (
    <Card className="bg-[var(--color-surface-elevated)]/80 border border-white/10 overflow-hidden">
      {/* Filters bar */}
      <div className="p-4 border-b border-white/5 flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full bg-[var(--color-surface)] border border-white/10 rounded-xl pl-9 pr-4 py-2 text-sm focus:border-white/20 focus:outline-none text-white"
          />
        </div>
        <select
          value={sectorFilter}
          onChange={(e) => onSectorChange(e.target.value)}
          className="bg-[var(--color-surface)] border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:border-white/20 focus:outline-none"
        >
          ...
          <tbody className="divide-y divide-white/5">
            {filtered.map((c) => (
              <tr key={c.id} className="hover:bg-white/[0.02] transition-colors cursor-pointer group">
                ...
                <td className="px-6 py-4">
                  <span className="text-xs bg-white/5 text-slate-400 px-2 py-0.5 rounded">
                    {c.industry}
                  </span>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center gap-1.5 text-xs ${
                    ...
                    c.status === "Em Implantação" ? "text-amber-400" :
                                                    "text-slate-400"
                  }`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${
                      c.status === "Ativo"          ? "bg-emerald-400" :
                      c.status === "Em Implantação" ? "bg-amber-400" :
                                                      "bg-slate-400"
                    }`} />
                    {c.status}
                  </span>
        ...
        {filtered.map((c) => (
          <div key={c.id} className="p-4 flex flex-col gap-3 hover:bg-white/[0.01] transition-all">
            ...
            <div className="flex items-center justify-between text-xs">
              <span className="bg-white/5 px-2 py-0.5 rounded text-xs text-slate-400">{c.industry}</span>
