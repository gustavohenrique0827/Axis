...
export function PerformanceScoreChart(props: { performanceData: any[] }) {
  return (
    <Card className="lg:col-span-2 p-5 bg-[var(--color-surface-elevated)]/80 border-white/5">
      ...
      <div className="h-[220px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={props.performanceData}>
            ...
            />
            <Bar dataKey="avgScore" fill="#94a3b8" radius={[4, 4, 0, 0]} barSize={24} name="Score IA" />
            <Line type="monotone" dataKey="conversionRate" stroke="#10b981" strokeWidth={2} dot={{ fill: "#10b981", r: 3 }} name="Conversão %" />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
      <p className="mt-3 text-xs text-slate-600 text-center">A cada 10pts de Score IA, a probabilidade de fechamento sobe ~4%.</p>
    </Card>
  );
}
...
export function HotLeadsPanel(props: { leads: any[]; hotLeads: any[] }) {
  ...
  return (
    <Card className="p-5 bg-[var(--color-surface-elevated)]/80 border-white/5 flex flex-col">
      ...
      {hotLeads.length === 0 ? (
        ...
        <div className="space-y-2 flex-1 min-h-0 overflow-y-auto scrollbar-none">
          {hotLeads.map((lead: any) => (
            ...
            >
              <div className="w-7 h-7 rounded-full bg-white/10 border border-white/10 flex items-center justify-center text-[9px] font-medium text-slate-300 shrink-0">
                {(lead.name || "?")[0]?.toUpperCase()}
              </div>
              <div className="min-w-0 flex-1">
                <div className="text-[11px] font-medium text-white truncate">{lead.name}</div>
                <div className="text-[9px] text-slate-500 truncate">{lead.company}</div>
              </div>
              <div className="text-[10px] text-slate-300 font-mono shrink-0">{lead.value || "—"}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="mt-4 pt-4 border-t border-white/5 grid grid-cols-2 gap-2 shrink-0">
        <div className="p-3 bg-white/[0.03] rounded-xl border border-white/5 text-center">
          ...
          <div className="text-[10px] text-slate-500">Ganhos</div>
        </div>
        <div className="p-3 bg-white/[0.03] rounded-xl border border-white/5 text-center">
          ...
          <div className="text-[10px] text-slate-500">Negociação</div>
        </div>
      </div>
    </Card>
  );
...
export function LeadsVolumeChart(props: { performanceData: any[] }) {
  return (
    <Card className="p-5 bg-[var(--color-surface-elevated)]/80 border-white/5">
      <h3 className="text-sm text-slate-400 flex items-center gap-2 mb-4">
        <Users className="w-4 h-4 text-slate-400" /> Volume de Leads Prospectados por Mês
      </h3>
      <div className="h-[140px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={props.performanceData}>
            ...
            <Area type="monotone" dataKey="leads" stroke="#94a3b8" fillOpacity={1} fill="url(#colorLeads)" strokeWidth={2} name="Leads" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </Card>
