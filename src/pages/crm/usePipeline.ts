...
export function usePipeline() {
  ...
  const tenantsList = useMemo(() => {
    ...
    return Array.from(
      new Set([
        ...
        ...(leads ?? []).map((l) => l.tenantName).filter(Boolean),
      ])
    );
  ...
  const sellers = useMemo(
    () => ["Todos", ...Array.from(new Set((leads ?? []).map((l) => l.seller).filter(Boolean)))],
    [leads]
  );

  const allSellersFullList = useMemo(
    () =>
      Array.from(
        new Set([
  ...
  const filteredItemsList = useMemo(() => {
    return (leads ?? []).filter((item) => {
      ...
      const matchesPipeline =
        currentPipeline === "sdr" && isSdrEnabled
          ? item.pipelineId === "sdr"
  ...
  const analyticsData = useMemo(() => {
    return activePipelineStages.map((s: any) => ({
      name: s.name,
  ...
  const totalValueSum = filteredItemsList.reduce((sum: number, item: any) => {
  ...
  const closedWonCount = filteredItemsList.filter(
    (l: any) => l.stageId === "5" || l.stageId === "sdr-5" || l.status === "Fechado"
  ).length;
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text(
      `Relatório de Pipeline - ${currentPipeline === "sdr" ? "SDR" : "Comercial"}`,
      14,
      22
    );
    doc.setFontSize(10);
    ...
    const tableData: string[][] = filteredItemsList.map((l: any) => [
      String(l?.name ?? ""),
      String(l?.company ?? ""),
      String(activePipelineStages.find((s: any) => s.id === l?.stageId)?.name ?? "Sem Fase"),
      String(l?.value ?? ""),
      String(l?.seller ?? ""),
    ]);
  ...
  const handleExportIAResume = (e: any, lead: any) => {
    ...
    doc.text(
      `Empresa: ${lead?.company ?? ""} | Gerado em: ${new Date().toLocaleDateString()}`,
      14,
      28
    );
    ...
    const splitSummary = doc.splitTextToSize(
      lead?.iaSummary || "Nenhuma análise de IA disponível para este lead.",
      170
    );
    doc.text(splitSummary as any, 14, 50);
    ...
    if (leadTasks.length === 0) {
      ...
      autoTable(doc, {
        ...
        body: tableData,
        theme: "grid",
        headStyles: { fillColor: [37, 99, 235] as any },
        styles: { fontSize: 9 },
      });
    }

    doc.save(`Resumo_IA_${String(lead?.name ?? "").replace(/\s+/g, "_")}.pdf`);
  ...
  const handleTransferToComercial = (e: any, lead: any) => {
    ...
    // compat: o schema real de Task pode divergir (desc/status/etc). Mantém como any para evitar TS travar.
    addTask({
      ...
      related: lead?.name ?? "",
      priority: "Média",
      seller: lead?.seller ?? user?.name ?? "Closer",
    } as any);
