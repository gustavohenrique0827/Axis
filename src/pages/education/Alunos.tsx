...
export default function Alunos() {
  ...
  const handleAddGrade = (e: React.FormEvent) => {
    ...
    // Cannot update directly without setStudents from useData matching the type, 
    // but the system is currently "em construção" so we just toast for now.
    toast.success("Nota registrada (Local, precisa integrar Supabase para Grades)");
  };

  const removeGrade = (index: number) => {
    if (!selectedStudent) return;
    // Same note about grades
    toast.info("Nota removida (Local)");
  };
