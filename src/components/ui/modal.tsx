...
export function Modal({ isOpen, onClose, title, children, footer, maxWidth = "max-w-md", position = "center" }: ModalProps) {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex ${position === 'right' ? 'justify-end' : 'items-center justify-center p-4 sm:p-6'}`}>
      ...
      {/* Modal Dialog */}
      <div 
        className={`relative w-[95vw] sm:w-full ${maxWidth} bg-[#111827] shadow-2xl overflow-hidden flex flex-col transform transition-all ${position === 'right' ? 'h-full border-l border-white/10 rounded-l-2xl max-h-screen animate-in slide-in-from-right duration-300' : 'border border-white/10 rounded-xl max-h-[90vh] animate-in zoom-in-95 duration-200'}`}
        role="dialog"
        aria-modal="true"
      >
        {/* Header */}
